...
async function entrarMaestra(claveAuto) {
  ...
  if (!clave) {
    if (!esAuto) mostrarErrorMaestra("Escribe tu clave.");
    return;
  }
  ...
  try {
    ...
    guardarSesionMaestraCompartida(clave, maestraIdActual, nombreMaestra);
...
async function guardarNuevaClaveMaestra() {
  ...
  try {
    ...
    guardarSesionMaestraCompartida(nueva, maestraIdActual, nombreMaestra);
...
// Si ya inició sesión antes (en el Portal de Maestras o en cualquier otra
// pantalla que use la misma clave), entra directo a su lista de alumnas sin
// pedirle la clave otra vez.
const sesionCompartidaChat = leerSesionMaestraCompartida();
if (sesionCompartidaChat && sesionCompartidaChat.clave) {
  entrarMaestra(sesionCompartidaChat.clave);
}
