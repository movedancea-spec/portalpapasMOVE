...
function renderPerfil(datos) {
  ...
  (datos.perfil || [])
    ...
    .forEach((f) => {
      ...
      // Justo debajo de "Tu clase" mostramos el objetivo técnico del
      // MES EN CURSO de cada grupo real de la alumna (puede ser más
      // de uno si toma varias clases) — nunca el de un mes anterior,
      // para que siempre refleje en qué está trabajando la clase
      // ahora mismo.
      if (f.campo === "CLASE") {
        (datos.objetivosMensuales || []).forEach((obj) => {
          if (!obj || !obj.objetivo) return;
